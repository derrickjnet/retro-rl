...
class StateEncoder:
  def __init__(self, sess, num_actions, num_images, embedder_dir=os.environ.get('RETRO_STATE_EMBEDDER_DIR'), embedder_model_scope=os.environ.get('RETRO_STATE_EMBEDDER_MODEL_SCOPE', 'state_embedder'), embedder_nfilters=int(os.environ.get('RETRO_STATE_EMBEDDER_NFILTERS',32)), embedder_embedding_size=int(os.environ.get('RETRO_STATE_EMBEDDER_EMBEDDING_SIZE',256)), predictor_dir=os.environ.get('RETRO_STATE_PREDICTOR_DIR'), predictor_model=os.environ.get('RETRO_PREDICTOR_MODEL'), predictor_model_scope=os.environ.get('RETRO_STATE_PREDICTOR_MODEL_SCOPE', 'state_predictor'), predictor_nfilters=int(os.environ.get('RETRO_STATE_PREDICTOR_NFILTES', 32)), predictor_embedding_size=int(os.environ.get('RETRO_STATE_PREDICTOR_EMBEDDING_SIZE', 256)), predictor_replay_size=int(os.environ.get("RETRO_STATE_PREDICTOR_REPLAY_SIZE", 10000)), predictor_batch_size=int(os.environ.get("RETRO_STATE_PREDICTOR_BATCH_SIZE", 64))):
    ...
    if self.predictor_dir:
      ...
      if predictor_model == "autoencoder": 
        predictor_model_builder = lambda predictor_rescaled_obses, predictor_actions: self.build_autoencoder_predictor_model(predictor_rescaled_obses, predictor_actions)
      elif predictor_model == "temporal-autoencoder": 
        predictor_model_builder = lambda predictor_rescaled_obses, predictor_actions: self.build_temporal_autoencoder_predictor_model(predictor_rescaled_obses, predictor_actions)
      elif predictor_model == "inverse-forward": 
      ...
      elif predictor_model == "forward": 
        predictor_model_builder = lambda predictor_rescaled_obses, predictor_actions: self.build_forward_predictor_model(predictor_rescaled_obses, predictor_actions)
      elif predictor_model == "inverse": 
        predictor_model_builder = lambda predictor_rescaled_obses, predictor_actions: self.build_inverse_predictor_model(predictor_rescaled_obses, predictor_actions)
      else:
        assert False
      ...
      with tf.variable_scope(predictor_model_scope + "_train"):
        ...
        self.predictor_train_step = tf.train.AdamOptimizer(learning_rate=0.00001*self.predictor_batch_size).minimize(self.predictor_train_loss, global_step = self.predictor_train_global_step)
        self.predictor_replay_buffer = []
        self.predictor_replay_size = predictor_replay_size
        self.last_predictor_train_loss_value = sys.float_info.max
  ...
  def build_autoencoder_predictor_model(self, predictor_model_rescaled_obses, predictor_actions):
    with tf.variable_scope(self.predictor_model_scope, reuse=tf.AUTO_REUSE):
      ...
      predictor_model_embedding = self.predictor.encoder(predictor_model_rescaled_obses, activation=tf.nn.relu, embedding_activation=None)
      predictor_model_outputs = self.predictor.decoder(predictor_model_embedding)
      ...
      predictor_embedding_loss = self.predictor.embedding_loss(predictor_model_embedding)
  ...
  def build_temporal_autoencoder_predictor_model(self, predictor_model_rescaled_obses, predictor_actions):
    with tf.variable_scope(self.predictor_model_scope, reuse=tf.AUTO_REUSE):
    ...
    with tf.variable_scope(self.predictor_model_scope, reuse=tf.AUTO_REUSE):
      predictor_model_outputs = self.predictor.decoder(predictor_embedding)
    with tf.variable_scope(self.predictor_model_scope + "_internal", reuse=tf.AUTO_REUSE):
      predictor_reconstruction_errors = self.predictor.reconstruction_errors(predictor_model_rescaled_obses[:,:,:,-1:], predictor_model_outputs)
      predictor_reconstruction_loss = tf.reduce_mean(predictor_reconstruction_errors)
      predictor_embedding_loss = tf.reduce_sum([self.predictor.embedding_loss(predictor_model_embedding) for predictor_model_embedding in predictor_model_embeddings])
    with tf.variable_scope(self.predictor_model_scope + "_internal", reuse=tf.AUTO_REUSE):
      predictor_extras = tf.zeros(0)
      predictor_rewards = 10 * tf.sqrt(predictor_reconstruction_errors / 84 / 84)
      predictor_train_loss = predictor_reconstruction_loss + predictor_embedding_loss
  ...
  def encode(self, obses, actions):
    ...
    if self.predictor_dir:
      ...
      print("STATE_ENCODER_PREDICTOR_EVAL: timestamp=%s step=%s rewards=%s actions=%s extras=%s duration=%ssec" % (predictor_eval_stop_timestamp, self.encoder_step, eval_predictor_reward_values, actions, eval_predictor_extra_values.tolist(), (predictor_eval_stop_timestamp - predictor_eval_start_timestamp).total_seconds()))
      ...
      if self.encoder_step % self.predictor_batch_size == 0 and len(self.predictor_replay_buffer) >= self.predictor_batch_size:
        ...
        self.last_predictor_train_loss_value = predictor_train_loss_value
        predictor_train_stop_timestamp = datetime.datetime.now()
        print("STATE_ENCODER_PREDICTOR_TRAIN: timestamp=%s step=%s train_loss=%s duration=%ssec" % (predictor_train_stop_timestamp, self.encoder_step, predictor_train_loss_value, (predictor_train_stop_timestamp - predictor_train_start_timestamp).total_seconds()))
    else:
