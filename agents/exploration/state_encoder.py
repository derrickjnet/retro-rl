...
class StateEncoder:
  def __init__(self, sess, num_actions, num_images, embedder_dir=os.environ.get('RETRO_STATE_EMBEDDER_DIR'), embedder_model_scope=os.environ.get('RETRO_STATE_EMBEDDER_MODEL_SCOPE', 'state_embedder'), embedder_nfilters=int(os.environ.get('RETRO_STATE_EMBEDDER_NFILTERS',32)), embedder_embedding_size=int(os.environ.get('RETRO_STATE_EMBEDDER_EMBEDDING_SIZE',256)), predictor_dir=os.environ.get('RETRO_STATE_PREDICTOR_DIR'), predictor_model_scope=os.environ.get('RETRO_STATE_PREDICTOR_MODEL_SCOPE', 'state_predictor'), predictor_nfilters=int(os.environ.get('RETRO_STATE_PREDICTOR_NFILTES', 32)), predictor_embedding_size=int(os.environ.get('RETRO_STATE_PREDICTOR_EMBEDDING_SIZE', 256)), predictor_replay_size=int(os.environ.get("RETRO_STATE_PREDICTOR_REPLAY_SIZE", 20000)), predictor_batch_size=int(os.environ.get("RETRO_STATE_PREDICTOR_BATCH_SIZE", 1))):
    ...
    if self.predictor_dir:
      ...
      with tf.variable_scope(predictor_model_scope + "_train"):
        ...
        self.predictor_train_step = tf.train.AdamOptimizer(learning_rate=0.0001).minimize(self.predictor_train_loss, global_step = self.predictor_global_step)
  ...
  def encode(self, obses, actions):
    ...
    if self.predictor_dir:
      ...
      eval_predictor_reward_values, eval_predictor_extra_values, predictor_global_step_value  = self.sess.run([ self.eval_predictor_rewards, self.eval_predictor_extras, self.predictor_global_step ], { self.eval_predictor_model_obses:obses, self.eval_predictor_actions:actions })
      predictor_eval_stop_timestamp = datetime.datetime.now()
      print("STATE_ENCODER_PREDICTOR_EVAL: timestamp=%s step=%s rewards=%s actions=%s extras=%s duration=%ssec" % (predictor_eval_stop_timestamp, predictor_global_step_value, eval_predictor_reward_values, actions, eval_predictor_extra_values.tolist(), (predictor_eval_stop_timestamp - predictor_eval_start_timestamp).total_seconds()))
      predictor_rewards = eval_predictor_reward_values

      predictor_train_start_timestamp = datetime.datetime.now()
      self.predictor_replay_buffer.extend(zip(obses.tolist(), actions))
      ...
      train_obses, train_actions = zip(*[random.choice(self.predictor_replay_buffer) for i in range(self.predictor_batch_size * len(actions))])
      ...
      print("STATE_ENCODER_PREDICTOR_TRAIN: timestamp=%s step=%s train_loss=%s duration=%ssec" % (predictor_train_stop_timestamp, predictor_global_step_value, predictor_train_loss_value, (predictor_train_stop_timestamp - predictor_train_start_timestamp).total_seconds()))
    else:
