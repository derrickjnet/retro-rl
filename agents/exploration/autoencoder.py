...
class Autoencoder:
  ...
  def encoder(self, inputs, activation=tf.nn.relu, embedding_activation=tf.nn.relu, reuse=None):
    with tf.variable_scope("encoder", reuse=reuse):
      net = tf.layers.conv2d(inputs, self.nfilters, [6, 6], strides=2, padding='SAME', activation=activation, name="conv1")
      net = tf.layers.conv2d(net, self.nfilters, [6, 6], strides=2, padding='SAME', activation=activation, name="conv2")
      net = tf.layers.conv2d(net, self.nfilters, [6, 6], strides=2, padding='SAME', activation=activation, name="conv3")
      net = tf.layers.conv2d(net, self.nfilters, [6, 6], strides=2, padding='SAME', activation=activation, name="conv4")
  ...
  def decoder(self, embeddings, actiovation=tf.nn.relu, reuse=None):
    with tf.variable_scope("decoder", reuse=reuse):
      ...
      net = tf.layers.conv2d_transpose(net, self.nfilters, [6, 6], strides=2, padding='SAME', activation=activation, name="deconv4")
      net = tf.layers.conv2d_transpose(net, self.nfilters, [6, 6], strides=2, padding='SAME', activation=activation, name="deconv3")
      net = tf.layers.conv2d_transpose(net, self.nfilters, [6, 6], strides=2, padding='SAME', activation=activation, name="deconv2")                
  ...
  def embed(self, activation, embedding_activation, reuse=None):
    model_obses = self.observations(reuse=reuse) 
    model_rescaled_obses = self.observations_rescaled(model_obses, reuse=reuse)
    model_embeddings = self.encoder(model_rescaled_obses, activation=activation, embedding_activation=embedding_activation, reuse=reuse)
    return model_obses, model_rescaled_obses, model_embeddings

  def model(self, use_noisy=False, use_embedding_loss=False):
    model_obses, model_rescaled_obses, model_embeddings_original = self.embed(activation=tf.nn.sigmoid if use_noise else tf.nn.relu, embedding_activation=tf.nn.sigmoid if use_noisy else None)
    if use_noisy:
      model_embeddings = self.embeddings_noisy(model_embeddings_original)
    else:
      model_embeddings = model_embeddings_original
    model_outputs = self.decoder(model_embeddings, activation=tf.nn.sigmoid if use_noise else tf.nn.relu)
