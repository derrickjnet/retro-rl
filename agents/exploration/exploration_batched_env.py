...
class ExplorationBatchedEnv(BatchedWrapper):
    def __init__(self, batched_env, exploration_f, state_encoder=None, record_root_dir=os.environ['RETRO_RECORD']):
        ...
        self.num_envs = batched_env.num_sub_batches * batched_env.num_envs_per_sub_batch
    ...
    def action_metas(self, action_metas, sub_batch=0):
        env_base_idx = sub_batch*batched_env.num_envs_per_sub_batch
        for env_offset_idx in range(0, batched_env.num_envs_per_sub_batch):
    ...
    def step_wait(self, sub_batch=0):
        ...
        env_base_idx = sub_batch*batched_env.num_envs_per_sub_batch
        for env_offset_idx in range(0, batched_env.num_envs_per_sub_batch):
          final_reward = self.explorations[env_base_idx + env_offset_idx].step(self.actions[env_offset_idx], obses[env_offset_idx], rews[env_offset_idx], dones[env_offset_idx], infos[env_offset_idx], state_embeddings[env_offset_idx])
