...
class Exploration:
   def __init__(self, env_id, allow_backtracking=True, max_exploration_steps=None):
     self.env_id = env_id 
   ...
   def step(self, action, obs, reward, done, info, state_embedding): 
     ...
     if False and 'RETRO_RECORD' in os.environ:
       ...
       record_dir_name = os.environ['RETRO_RECORD'] + "/" + self.env_id + "-" + str(self.episode).zfill(4) + ".steps/"
