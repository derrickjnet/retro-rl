...
from anyrl.envs.base import AsyncEnv, BatchedEnv, BatchedAsyncEnv
        ...
        try:
            while True:
                ...
                if cmd == 'action_space':
                    ...
                    last_done = False
                    last_info = {}
                    #print("BATCHED_GYM_ENV_WORKER_TRY_RESET", last_done, last_info)
                    try:
                      last_obs = env.reset()
                    except:
                        print("BATCHED_GYM_ENV_WORKER_RESET_EXCEPTION: %s" % (traceback.format_exc(),))
                        env.close()
                        env = make_env()
                        last_obs = env.reset()
                    pipe.send(cls._sendable_observation(last_obs, obs_buf))
                elif cmd == 'step':
                    #print("BATCHED_GYM_ENV_WORKER_TRY_STEP", last_done, last_info)
                    try:
                      last_obs, last_rew, last_done, last_info = env.step(action)
                      if last_done:
                        try:
                          last_obs = env.reset()
                        except:
                          print("BATCHED_GYM_ENV_WORKER_STEP_RESET_EXCEPTION: %s" % (traceback.format_exc(),))
                          env.close()
                          env = make_env()
                          last_obs = env.reset()
                    except:
                      print("BATCHED_GYM_ENV_WORKER_STEP_EXCEPTION: %s" % (traceback.format_exc(),))
                      env.close()
                      env = make_env()
                      last_done = True
                      last_info = {}
                      last_obs = env.reset()
                    pipe.send((cls._sendable_observation(last_obs, obs_buf), last_rew, last_done, last_info))
                elif cmd == 'close':
                    return
                else:
